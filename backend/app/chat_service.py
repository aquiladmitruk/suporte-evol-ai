...
class ChatService:
    ...
    async def process_message(
        ...
        """
        # 1. Recuperar chunks via RAGService (pode lançar RuntimeError do Qdrant)
        chunks = await self._rag_service.retrieve_chunks(user_message)

        # 2. Construir system prompt com os chunks recuperados
        system_prompt = self._prompt_builder.build_system_prompt(chunks)

        # 3. Obter histórico do servidor
        server_history = self._session_store.get_history(session_id)

        # 4. Mesclar histórico: usar client_history se servidor estiver vazio
        history = server_history if server_history else client_history

        # 5. Construir lista de mensagens para o LLM
        messages = self._prompt_builder.build_messages(
            system_prompt, history, user_message
        )

        # 6. Chamar o LLM (pode lançar RuntimeError do LLM)
        response_text = await self._llm_client.complete(messages)

        # 7. Salvar mensagem do usuário e resposta no SessionStore
        ...
        # 8. Retornar ChatResponse com fontes dos chunks
